...
local function get_p2_character()
    local char_id = memory.readbyte(0xFF8800 + 0x382)
...
local dash_attack_delay = {
    ["Bulleta"]   = {forward = 5,  back = 5},
    ["Gallon"]    = {forward = 8,  back = 3},
    ["Victor"]    = {forward = 5,  back = 5},
    ["Zabel"]     = {forward = 1,  back = 1},
    ["Morrigan"]  = {forward = 12, back = 12},
    ["Anakaris"]  = {forward = 5,  back = 5},
    ["Felicia"]   = {forward = 8,  back = 8},
    ["Bishamon"]  = {forward = 6,  back = 3},
    ["Aulbath"]   = {forward = 6,  back = 0},
    ["Sasquatch"] = {forward = 2,  back = 1},
    ["Q-Bee"]     = {forward = 6,  back = 6},
    ["Lei-Lei"]   = {forward = 1,  back = 5},
    ["Lilith"]    = {forward = 5,  back = 5},
...
function make_input_sequence(_stick, _button, _delay_type, _delay_num)
 ...
 if _stick == "forward dash" or _stick == "back dash" then
    local char = get_p2_character()

    if char and dash_attack_delay[char] then
      local delay

      if _stick == "forward dash" then
        delay = dash_attack_delay[char].forward
      else
        delay = dash_attack_delay[char].back
      end
  ...
  if _stick == "forward dash cancel" then
    ...
    if char == "Sasquatch" then
        table.insert(_sequence, { "forward", _button })
    else
  ...
  elseif _button == "HP+HK" then
    ...
    table.insert(_sequence[#_sequence], "HK")
  elseif
    _stick == "forward dash" or
    _stick == "back dash"
  then
    table.insert(_sequence, {_button})
  elseif
     (_stick == "forward dash cancel" or
     _stick == "back dash cancel")
    and get_p2_character() ~= "Sasquatch"
  then
    table.insert(_sequence, {_button})
  else
